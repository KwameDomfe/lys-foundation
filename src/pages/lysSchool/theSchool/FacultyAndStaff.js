import Portrait from '../../../images/placeholders/regular_images/avatar-01.png'

export default function FacultyAndStaff() {
  return (
    <div id="SGMAtAGlance"
        className="flex flex-column items-center ml1-00"
    >
        <header>
            <h2 className="">
              Faculty and Staff Header
            </h2>
        </header>
    
        <div id="FacultyAndStaff"
            className="min-vh-50 h-100 flex flex-column justify-start ml1-00"
        >
            <hgroup className="mb2-00">
                <p className="lh-copy">Lorem ipsum dolor sit amet consectetur adipisicing elit. Sint, dicta nihil voluptatibus accusantium aspernatur beatae eius nisi fugiat, consequatur quod minus nesciunt laboriosam libero, quibusdam perferendis ut rerum provident et.
                </p>
                <p className="lh-copy">Lorem ipsum dolor sit amet consectetur adipisicing elit. Sint, dicta nihil voluptatibus accusantium aspernatur beatae eius nisi fugiat, consequatur quod minus nesciunt laboriosam libero, quibusdam perferendis ut rerum provident et.
                </p>
            </hgroup>
            <div className="">
                <h1 className="f3-00">Preschool</h1>
                <div className="grid gtc12 ggap2-00 w-100 pr1-00 mb4-00 tc"
                >
                    <div className="gc1s3 gr1s1
                        flex items-center justify-center 
                    "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                    <div className="gc4s3 gr1s1
                        flex items-center justify-center
                        bg-black-20 "
                    >
                    <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                    <div className="gc7s3 gr1s1
                        flex items-center justify-center
                        
                        bg-black-50 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                    <div className="gc10s3 gr1s1
                        flex items-center justify-center
                        
                        bg-black-20 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                </div>
            </div>
            <div className="">
                <h1 className="f3-00">Lower Primary</h1>
                <div className="grid gtc12 ggap2-00 w-100 pr1-00 mb4-00 tc"
                >
                    <div className="gc1s3 gr1s1
                        flex items-center justify-center 
                        bg-black-50 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                    <div className="gc4s3 gr1s1
                        flex items-center justify-center
                        bg-black-20 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                    <div className="gc7s3 gr1s1
                        flex items-center justify-center
                        
                        bg-black-50 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                    <div className="gc10s3 gr1s1
                        flex items-center justify-center
                        
                        bg-black-20 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                </div>
            </div>
            <div className="">
                <h1 className="f3-00">Upper Primary</h1>
                <div className="grid gtc12 ggap2-00 w-100 pr1-00 mb4-00 tc"
                >
                    <div className="gc1s3 gr1s1
                        flex items-center justify-center 
                        bg-black-50 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                    <div className="gc4s3 gr1s1
                        flex items-center justify-center
                        bg-black-20 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                    <div className="gc7s3 gr1s1
                        flex items-center justify-center
                        
                        bg-black-50 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                    <div className="gc10s3 gr1s1
                        flex items-center justify-center
                        
                        bg-black-20 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                </div>
            </div>
            <div className="">
                <h1 className="f3-00">Junior High</h1>
                <div className="grid gtc12 ggap2-00 w-100 pr1-00 mb4-00 tc"
                >
                    <div className="gc1s3 gr1s1
                        flex items-center justify-center 
                        bg-black-50 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                    <div className="gc4s3 gr1s1
                        flex items-center justify-center
                        bg-black-20 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                    <div className="gc7s3 gr1s1
                        flex items-center justify-center
                        
                        bg-black-50 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                    <div className="gc10s3 gr1s1
                        flex items-center justify-center
                        
                        bg-black-20 "
                    >
                        <figure>
                            <img src={Portrait} 
                                alt=""
                                className="h-100"
                            />
                            <figcaption className="pa1-00 mt1-00 bg-black-50">
                                <h3 className="mv1-00">Auntie Winnie</h3>
                                <p className="mb2-00">English</p>
                                <a className="white-90 pv0-25 ph0-50 ba br0-25" 
                                    href="/"
                                >
                                        Explore More
                                </a>
                            </figcaption>
                        </figure>
                    </div>
                </div>
            </div>

        </div>

        <footer>
            <h2 className="">
               Faculty and Staff Footer
            </h2>
        </footer>

    </div>
  )
}
